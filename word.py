...
class WordLM(object):
    ...
    def __init__(self, args, is_training, is_testing=False):
        ...
        lm_cell = tf.contrib.rnn.MultiRNNCell([cell] * args.num_layers)
        ...
        lm_outputs, lm_state = tf.contrib.legacy_seq2seq.rnn_decoder(lm_inputs, self._initial_lm_state, lm_cell, loop_function=None, scope='rnnlm')

        #lm_outputs = tf.concat(1, lm_outputs)
        #lm_outputs = tf.reshape(lm_outputs, [-1, rnn_size])
        lm_outputs = tf.reshape(tf.concat(lm_outputs, 1), [-1, rnn_size])
